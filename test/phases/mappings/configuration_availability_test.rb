...
class ConfigurationAvailabilityTest < DotfilesTestCase
  ...
  def test_targets_returns_current_mappings_before_planning
    ...
    availability = described_class.new(load_mappings: -> { [mapping(link_source, link_target)] })

    assert_equal [link_target], availability.targets
  end
